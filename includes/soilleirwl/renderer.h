...
typedef void (*SWL_RENDERER_COPY_FROM_TEXTURE)(swl_renderer_t *renderer, swl_texture_t *text, void *dst, uint32_t h, uint32_t w, uint32_t x, uint32_t y, uint32_t format);

struct swl_renderer {
	SWL_RENDER_BEGIN begin;
	SWL_RENDERER_COPY_FROM copy_from;
	SWL_RENDERER_COPY_FROM_TEXTURE copy_from_texture;
	SWL_RENDER_END end;
